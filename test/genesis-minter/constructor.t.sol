...
contract GenesisMinter_Constructor_Test is BaseTest {
    ...
    /**
     * @dev test the correct contract creation of GenesisMinter and setup of AccessControl roles
     */
    function test_Constructor() external {
        ...
        address _minterStorageImplAddr = bytes32ToAddress(_minterStorageImpl);
        ...
        assertEq(bytes32ToAddress(entries[1].topics[1]), _minterStorageImplAddr);
        ...
        assertEq(entries[3].topics[1], minterImpl.DEFAULT_ADMIN_ROLE());
        ...
        assertEq(entries[4].topics[1], minterImpl.MINTER_ROLE());
        ...
        require(minterImpl.hasRole(DEFAULT_ADMIN_ROLE, owner), "owner doesn't have DEFAULT_ADMIN_ROLE");

        // Contract owner does not have the MINTER_ROLE
        require(!minterImpl.hasRole(MINTER_ROLE, owner));

        // minter does not have the DEFAULT_ADMIN_ROLE
        require(!minterImpl.hasRole(DEFAULT_ADMIN_ROLE, minter));

        // minter has the MINTER_ROLE
        require(minterImpl.hasRole(MINTER_ROLE, minter));

        // crafter doesn't have the MINTER_ROLE
        require(!minterImpl.hasRole(MINTER_ROLE, crafter));
        ...
        minterImpl.revokeRole(DEFAULT_ADMIN_ROLE, owner);

        // DEFAULT_ADMIN_ROLE user can revoke a role
        vm.prank(owner);
        minterImpl.revokeRole(MINTER_ROLE, minter);

        // DEFAULT_ADMIN_ROLE user can grant a role to another user
        vm.prank(owner);
        minterImpl.grantRole(MINTER_ROLE, bob);
    }
