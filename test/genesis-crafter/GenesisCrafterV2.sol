...
/**
 * @title GenesisCrafterV2
 *
 * @notice GenesisCrafterV2 is a UUPS proxy implementing crafting mechanisms & rules for all GenesisChampion tokens
 * @dev This V2 is just an example for upgrade tests
 */
contract GenesisCrafterV2 is IGenesisCrafter, GenesisUpgradeable {
    ...
    /// @notice emitted when minter has changed
    ...
    function viewMaxCraftCount(address collection, uint256 id) external returns (uint256) {
        if (!craftCounters[collection][id].initialized) return IGenesisChampion(collection).defaultMaxCraftCount();
        else return IGenesisChampion(collection).defaultMaxCraftCount();
    }
     ...
     * @param owner of the token
     */
    function _validateCraftConditions(address collection, uint256 id, address owner)
        internal
        returns (uint256, uint256)
    {
        ...
        if (champ.ownerOf(id) != owner) revert Errors.CallerNotOwner(collection, id);
